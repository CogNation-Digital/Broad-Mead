        ...
        <div class="table-responsive">
            <table class="table table-bordered">
                ...
                <tbody>
                    <?php
                    $n = 1;
                    $__shifts__query = "SELECT * FROM `_shifts` WHERE (CandidateID = '$CandidateID') AND ShiftDate BETWEEN '$FromDate' AND '$ToDate'";
                    ...
                    while ($row = $__shifts__stmt->fetchObject()) { ?>
                        ...
                        <tr data-id="<?php echo $row->ShiftID; ?>">
                            <td><?php echo $n++; ?></td>
                            <td style="display: none;"><input class="form-check-input checkbox-item" type="checkbox" data-id="<?php echo $row->ShiftID; ?>" value="view_vacancy/?VacancyID=<?php echo $row->VacancyID; ?>&hasCandidateID=<?php echo $CandidateID ?>&isTab=Shifts" VacancyID="<?php echo $row->VacancyID; ?>" id="flexCheckDefault<?php echo $row->id; ?>"></td>
                            ...
                            <td>
                                <div class="dropdown">
                                    <a class="avtar avtar-s btn-link-secondary dropdown-toggle arrow-none" href="#" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="ti ti-dots-vertical f-18"></i></a>
                                    <div class="dropdown-menu dropdown-menu-end">
                                        <a class="dropdown-item edit select-entry-row" href="#">
                ...
                <?php if (IsCheckPermission($USERID, "CREATE_SHIFT")) : ?>
                    <form method="POST" enctype="multipart/form-data">
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label class="form-label" for="email">Candidate</label>
                                <select name="CandidateID" class="form-control">
                                    ...
                                    $stmt->execute();
                                    while ($row = $stmt->fetchObject()) {
                                        if ($CandidateID ==  $row->CandidateID) {
                                            echo '<option selected value="' . $row->CandidateID . '">' . $row->Name . '</option>';
                                        }
                                    }
                                    ?>
                                </select>
