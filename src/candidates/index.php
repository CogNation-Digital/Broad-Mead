        ...
        if (!isset($error_message)) {
            foreach ($candidate_ids as $candidate_id) {
                ...
                try {
                    ...
                    if (!$candidate) {
                        $stmt = $db_1->prepare("SELECT id, CONCAT(first_name, ' ', last_name) as Name, email as Email FROM candidates WHERE id = ?");
                        $stmt->execute([$candidate_id]);
                        $candidate = $stmt->fetch(PDO::FETCH_OBJ);
                    }
                    ...
                    if ($candidate && filter_var($candidate->Email, FILTER_VALIDATE_EMAIL)) {
                        ...
                        $mail->Port = $smtp_port;
                        $mail->Timeout = 30; // Set a timeout for sending
                        $mail->SMTPOptions = array( // Options to bypass SSL certificate issues (use with caution in production)
                            'ssl' => array(
                                'verify_peer' => false,
    ...
    try {
        ...
        if ($exportMode === 'candidates') {
            ...
            if (!empty($data_to_export)) {
                // Define specific headers for candidates export
