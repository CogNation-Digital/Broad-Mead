...
// Database connections - MOVED TO TOP
try {
...
// FIXED: Process mailshot AFTER database connections are established
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $mode === 'mailshot') 
    ...
    if (empty($_POST['selected_candidates'])) {
        ...
        // Only proceed if SMTP test passed
            ...
            foreach ($candidate_ids as $candidate_id) {
                try {
                    ...
                    if ($candidate && filter_var($candidate->Email, FILTER_VALIDATE_EMAIL)) {
                        ...
                        // Add small delay to prevent overwhelming SMTP server
...
if (!empty($console_logs)) {
    ...
    <?php 
    // Temporary bypass for permission check
    $has_kpi_permission = true; // Assuming user has permission for now
...
<!-- part 2 -->
                    ...
                    <?php
                    // This section is for 'candidates' and 'mailshot' modes, not 'kpi'
                    if ($mode !== 'kpi') {
                        try {
                    ...
                    ?>
                    <?php if ($mode !== 'kpi'): // Only show results info for candidates/mailshot modes ?>
                    ...
                    <?php endif; ?>
<!-- part 3 -->
                    <?php if ($mode !== 'kpi' && $total_results > 0): // Only show table for candidates/mailshot modes ?>
                        <?php if ($mode === 'mailshot'): ?>
                            ...
                            <table class="table table-bordered table-striped" id="editableTable">
                                <thead class="thead-light">
                                    <tr>
                                        ...
                                        <th>Distance</th>
                                        <?php endif; ?>
                                        <th>Created By</th> <?php // This column is now always shown for consistency ?>
                                        <th>Date</th>
                                        <?php if ($mode === 'candidates'): ?>
                                        <th>Actions</th>
                                        <?php endif; ?>
                    ...
                    <?php elseif ($mode !== 'kpi'): // No candidates found for candidates/mailshot modes ?>
            ...
            <div class="row" style="margin-bottom: 20px;">
                <div class="col-md-3">
                    ...
                    <select name="kpi_period" id="kpi_period" class="form-control">
                        <option value="current_week" <?php if($kpi_period=='current_week') echo 'selected'; ?>>Current Week</option>
                        <option value="last_week" <?php if($kpi_period=='last_week') echo 'selected'; ?>>Last Week</option>
                        <option value="current_month" <?php if($kpi_period=='current_month') echo 'selected'; ?>>Current Month</option>
                        <option value="last_month" <?php if($kpi_period=='last_month') echo 'selected'; ?>>Last Month</option>
                        <option value="current_quarter" <?php if($kpi_period=='current_quarter') echo 'selected'; ?>>Current Quarter</option>
                        <option value="current_year" <?php if($kpi_period=='current_year') echo 'selected'; ?>>Current Year</option>
                        <option value="custom" <?php if($kpi_period=='custom') echo 'selected'; ?>>Custom Range</option>
                    </select>
                </div>
                <div class="col-md-3">
                    <label for="kpi_start_date">Start Date</label>
                    <input type="date" name="kpi_start_date" id="kpi_start_date" class="form-control" value="<?php echo htmlspecialchars($kpi_start_date); ?>">
                </div>
                <div class="col-md-3">
                    <label for="kpi_end_date">End Date</label>
                    <input type="date" name="kpi_end_date" id="kpi_end_date" class="form-control" value="<?php echo htmlspecialchars($kpi_end_date); ?>">
                </div>
                        ...
                        <table class="table table-bordered table-striped" id="detailedKpiTable">
                            ...
                            <tbody>
                                ...
                                foreach ($kpi_data['detailed_candidates'] as $candidate) {
                                    ...
                                    // Aggregate status counts for summary row
    ...
    <script>
        // Enable/disable date inputs based on KPI period selection
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function toggleDateInputs() {
                ...
                // Also ensure readonly is false if it was set elsewhere, though disabled should be sufficient
                startDateInput.readOnly = !isCustom;
                endDateInput.readOnly = !isCustom;

                console.log('Start Date disabled:', startDateInput.disabled, 'End Date disabled:', endDateInput.disabled);
        ...
        function exportTableToCSV(filename) {
            ...
            table.querySelectorAll('tbody tr').forEach(row => {
                ...
                row.querySelectorAll('td').forEach(cell => {
                    // Clean up cell text, remove commas, newlines, and HTML
                    let cellText = cell.innerText.trim().replace(/"/g, '""');
                    if (cellText.includes(',') || cellText.includes('\n')) {
                        cellText = `"${cellText}"`; // Enclose in quotes if contains commas or newlines
                    }
            ...
            // Get footer data (Status Percentages)
