...
if (isset($_POST['submit'])) {
    ...
    if (empty($user) || empty($StartDate) || empty($EndDate) || empty($Description)) {
        ...
        if ($count == 0) {
            $response = "KPI ID does not exist. Please try again.";
            $error = 1;
        } else {
            $query = "UPDATE `_kpis` 
                      SET `UserID` = :userID, 
            ...
            if ($stmt->execute()) {
                $Modification = "Updated key performance indicator.";
                $Notification = "$NAME updated the key performance indicator.";
                LastModified($KpiID, $USERID, $Modification);
                Notify($USERID, $ClientKeyID, $Notification);
                $response = "KPI updated successfully.";
            } else {
                $response = "Something went wrong. Please try again";
                $error = 1;
            }
...
$query = "SELECT * FROM `_kpis` WHERE `KpiID` = :KpiID";
$stmt = $conn->prepare($query);
$stmt->bindParam(':KpiID', $KpiID, PDO::PARAM_STR);
$stmt->execute();
$kpi_data = $stmt->fetchObject();
?>
                            ...
                            <div class="d-flex align-items-center justify-content-between">
                                ...
                                <div class="flex-shrink-0">
                                    <?php if (isset($_GET['Add_KPIs'])) : ?>
                                        <a href="<?php echo $LINK; ?>/edit_weekly_kpis/?ID=<?php echo $KpiID; ?>" class="btn btn-sm btn-light-secondary"><i class="ti ti-edit"></i> Edit Details</a>
                                    <?php else : ?>
                                        <a href="<?php echo $current_url; ?>&Add_KPIs=true" class="btn btn-sm btn-light-secondary"><i class="ti ti-edit"></i> Edit KPIs</a>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                        <div class="card-body">
                            <?php if (IsCheckPermission($USERID, "EDIT_KPI")) : ?>
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    <div class="mb-3 mt-3"><label class="form-label">Search and select the consultant</label>
                                                        <select name="user" class="select-input" style="padding: 20px;">
                                                            ...
                                                            while ($r = $q->fetchObject()) { ?>
                                                                <option <?php echo ($r->UserID == $kpi_data->UserID) ? "selected" : ""; ?> value='<?php echo $r->UserID; ?>'><?php echo $r->Name; ?></option>;
                                                            <?php } ?>
                                                        </select>
                                                    </div>
                                                </div>
                                                    ...
                                                    <div class="row">
                                                        <div class="col-sm-6" style="display: none;">
                                                            <div class="mb-3"><label class="form-label">Starts</label>
                                                                <input type="date" value="<?php echo $kpi_data->StartDate; ?>" class="form-control" name="StartDate">
                                                            </div>
                                                        </div>
                                                        <div class="mb-3">
                                        ...
                                        <div class="col-12">
                                            <div class="card shadow-none border mb-0">
                                                <div class="card-body">
                                                    ...
                                                    <div class="mb-3">
                                                        <textarea name="Description" class="form-control"><?php echo $kpi_data->Description; ?></textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
